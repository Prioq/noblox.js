...
// Docs
/**
 * ✅ Send an HTTP request to the specified URL with options.
 * @category Utility
 * @alias http
...
async function http(url, opt) {
  ...
  try {
    const response = await requestAsync({ url, ...opt });
    return response;
  } catch (e) {
...
exports.func = function (args) {
  return new Promise((resolve, reject) => {
    ...
    opt.followRedirect = function (res) {
      if (
        !args.ignoreLoginError &&
        res.headers.location &&
        (res.headers.location.startsWith('https://www.roblox.com/newlogin') ||
          res.headers.location.startsWith('/Login/Default.aspx'))
      ) {
        return false;
      }
      return follow;
    ...
    const attemptRequest = (retryCount = 0) => {
      http(args.url, opt)
        .then(function (res) {
            ...
            if (res.statusCode === 403) {
              ...
              if (
                ...
                if (depth >= 3) {
                  return reject(
                    new Error(
                      'Tried ' +
                        depth +
                        ' times and could not refresh XCSRF token successfully'
                    )
                  );
                }
                ...
                if (token) {
                  ...
                  return reject(new Error('Could not refresh X-CSRF-TOKEN'));
                }
                return;
              }
            } else {
          ...
          return resolve(full ? res : res.body);
        })
        .catch(function (err) {
          if (
            (err.code === 'ESOCKETTIMEDOUT' || err.code === 'ETIMEDOUT') &&
            retryCount < 3
          ) {
            // Retry the request
            retryCount++;
            console.warn(
              `Request timed out, retrying (${retryCount}/3)... Error: ${err.message}`
            );
            attemptRequest(retryCount);
          } else {
