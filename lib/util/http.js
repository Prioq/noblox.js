...
function http (url, opt) {
  ...
  // Only use session_only mode if proxy is not explicitly disabled
  // Direct Roblox connections require real cookie jars, not headers
  const proxyExplicitlyDisabled = opt && opt.proxy && opt.proxy.enabled === false
  if (settings.session_only && opt && opt.jar) {
    if (proxyExplicitlyDisabled) {
      // Create a real cookie jar for direct Roblox connections
      const realJar = request.jar()
      // Set cookie with domain=.roblox.com to work across all subdomains
      const cookieStr = '.ROBLOSECURITY=' + opt.jar.session + '; Domain=.roblox.com; Path=/; Secure; HttpOnly'
      const cookie = request.cookie(cookieStr)
      realJar.setCookie(cookie, 'https://www.roblox.com')
      opt.jar = realJar
    } else {
      // Use header-based authentication for proxied requests
      if (!opt.headers) {
        opt.headers = {}
      }
