      ...
      return getGeneralToken({}).then((token) => {
        return http({
          ...
          if (cookies) {
            ...
            fs.writeFile(cookieFile, JSON.stringify({ cookie: options.jar.session, time: Date.now() }), () => {
              return true
            })
          }
        })
  ...
  if (fs.existsSync(cookieFile)) {
    ...
    if (json.time + day > Date.now()) {
      ...
      try {
        ...
        // Stored relog failed, will try with given cookie
      }
    }
  }
  if (cookie) {
    ...
    try {
      ...
      // Relog failed, will throw error below
    }
  }
