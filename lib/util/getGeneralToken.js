...
function getGeneralToken (jar) {
  ...
  const httpOpt = {
    ...
    url: `//auth.${process.env['PROXY'] || settings.proxyDomain || "roblox.com"}/v2/logout`, // REQUIRES https. Thanks for letting me know, ROBLOX...
    options: {
  ...
  return http(httpOpt)
    .then(function (res) {
      ...
      throw new Error('Failed to get X-CSRF-TOKEN: ' + err.message)
    })
}
