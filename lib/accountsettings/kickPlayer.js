  ...
  try {
    ...
    if (blockSuccessful && errorType.type !== 'auth') {
      ...
      try {
        ...
        const cleanupVerified = await pollUntilUnblocked(jar, userId, apiUrl)
        if (cleanupVerified) {
          logger.info('Cleanup verified - user confirmed unblocked', { userId })
        } else {
          logger.warn('Cleanup completed but verification failed', { userId })
        }
      } catch (cleanupError) {
